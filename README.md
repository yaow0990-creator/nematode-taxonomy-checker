...
补充说明：本工具内置Nemaplex数据库离线快照，快照采集时间：2026年09月。
快照为固定版本数据。原网站后续更新的线虫分类、c-p值与取食功能信息，不会自动同步到本项目。
若使用者需要网站最新数据，可运行配套爬虫脚本重新抓取，替换data目录下本地json文件。
⚠️ 爬虫使用请遵守Nemaplex网站访问规则，禁止高频批量请求，避免对网站服务器造成负担。
